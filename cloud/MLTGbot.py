...
def characteristics_choice(callback):
    if callback.data == 'price':
        ...
        prices = []
        links = []
        for elem in smartphones_top:
            price, smartphone = elem[0], elem[1]
            ...
            prices.append(price)
            smart.append(smartphone)
            links.append(f"https://megamarket.ru/catalog/?q={smartphone.replace(' ', '%20')}&collectionId=12546")

        res_text = 'Вот ваш результат:\n\n'
        for i in range(len(smart)):
            res_text += f'{i + 1}. {smart[i]} ~~{prices[i]}руб.\n {links[i]} \n'
        print(smart)
